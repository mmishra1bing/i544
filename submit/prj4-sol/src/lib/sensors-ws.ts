  ...
  : Promise<Errors.Result<Record<string, string>>>
{
  try {
    ...
    if (responseData.isOk) {
      // Success Envelope
      const displayValue = displayFn(responseData.result);
      return Errors.okResult(displayValue);
    } else {
      // Failure Envelope
      const errorResult = new Errors.ErrResult(responseData.errors);
      return Errors.errResult(errorResult);
    }
  } catch (error) {
    const errorResult = new Errors.ErrResult(error);
    return Errors.errResult(errorResult);
  }
  ...
  : Promise<Errors.Result<PagedValues>>
{
  try{
    ...
    if (response.isOk) {
      // Success Envelope
      const values = response.result.map((item: { result: T }) => displayFn(item.result));
      ...
      // Failure Envelope
      const errorResult = new Errors.ErrResult(response.errors);
      return Errors.errResult(errorResult);
    }
  }catch(error){
    const errorResult = new Errors.ErrResult(error);
